...
七维表面适合“发现”：它逼迫团队逐项询问还缺什么观察。进入契约阶段时，统一写成本书的七字段契约：

\[
K = (I, O, X, S, E, P, U)
\]

其中 `I` 是调用、输入与初态，`O` 是 stdout/stderr，`X` 是退出码、信号或超时，`S` 是外部状态转移，`E/P/U` 分别约束环境、平台与未知；另以必填 `non_goals` 划定非目标。[第 3 章](ch03-behavior-contract.md)把这些观察编译成可执行契约。

这种映射有一个重要效果：任何“实现某功能”的任务，都可以逐字段追问承诺。若答案只有 `O`，任务还不是可安全委派的迁移切片。
...
```mermaid
flowchart LR
    L["Legacy Behavior<br/>规范、黑盒、测试、运行事实"] --> C["Behavior Contract<br/>I/O/X/S/E/P/U + non_goals"]
...
target_contract:
  behavior_intent:
  required_surface:          # I/O/X/S/E/P/U
  allowed_differences:
  non_goals:
  decision_owner:
unknowns:
...
2. **契约栏**：当前只处理一个行为意图，展示 `I/O/X/S/E/P/U`、`non_goals`、允许差异与停止条件。
...
- [ ] 将七维观察映射为 `I/O/X/S/E/P/U`，并填写 `non_goals`。
